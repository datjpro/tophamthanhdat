  ...
  return (
    <div className="space-y-10">
      <section className="space-y-4">
        <p className="mono-label text-accent">Project Chronology</p>
        <h1 className="section-title">Selected Artifacts</h1>
        <p className="max-w-3xl text-muted-foreground">
          Horizontal timeline layout with asymmetry inspired by stitch sample number five.
        </p>
      </section>
      ...
      <section className="timeline-scroll overflow-x-auto pb-6">
        <div className="flex min-h-[32rem] items-start gap-9 px-2">
          {filteredProjects.map((project, index) => (
            <TimelineCard key={project.slug} project={project} order={index + 1} />
          ))}
        </div>
      </section>
    </div>
...
function TimelineCard({ project, order }: { project: ProjectItem; order: number }) {
